...
export const VenueStep = () => {
	...
	const stringIsValidLength = (value: string | undefined): boolean =>
	...
	const formIsValid = useMemo(() => {
		...
		const rules = [
			...
			!!States[venue.state as keyof typeof States],
		];
	...
	return (
		<div className="space-y-6 max-w-3xl mx-auto">
			<Input
				...
				onChange={(e: ChangeEvent<HTMLInputElement>) => updateVenue({ name: e.target.value })}
			/>
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<Input
					...
					onChange={(e: ChangeEvent<HTMLInputElement>) => updateVenue({ email: e.target.value })}
				/>
				<Input
					label="Phone Number"
			...
			<div className="space-y-4">
				<Input
					...
					onChange={(e: ChangeEvent<HTMLInputElement>) => updateVenue({ addressLine1: e.target.value })}
				/>
				<Input
					label="Address Line 2"
					value={venue.addressLine2}
					placeholder="Unit #30"
					onChange={(e: ChangeEvent<HTMLInputElement>) => updateVenue({ addressLine2: e.target.value })}
				/>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<Input
					...
					onChange={(e: ChangeEvent<HTMLInputElement>) => updateVenue({ city: e.target.value })}
				/>
				<Select
					value={States[venue.state as keyof typeof States]}
					onChange={(state: string) => updateVenue({ state })}
				>
